...
static int rt5645_jack_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		...
		regmap_update_bits(rt5645->regmap, 0x91, 3 << 11, 3 << 11);
	...
	struct snd_soc_jack *btn_jack)
{
	...
	if (rt5645->btn_jack && rt5645->codec_type == CODEC_TYPE_RT5650) {
		...
		regmap_update_bits(rt5645->regmap, RT5645_GEN_CTRL2,
				0x1 << 4, 0x1 << 4);
		regmap_update_bits(rt5645->regmap, 0xfc, 0x1 << 8, 0x1 << 8);
		regmap_update_bits(rt5645->regmap, 0x0a, 0x1 << 5, 0x1 << 5);
		regmap_update_bits(rt5645->regmap, 0x0c, 0x1 << 15, 0);
	}
...
static int rt5645_parse_dt(struct rt5645_priv *rt5645, struct device *dev)
{
	...
	device_property_read_u32(dev,
        ...
        rt5645->pdata.jd_invert = device_property_read_bool(dev,
                                                            "realtek,jd-invert");
